...
class Evaluator(nn.Module, Callback):
    ...
    def __init__(
        ...
        self.test_metrics = ModuleList(self.validate_metrics(test_metrics))
        self.compute_on_cpu = compute_on_cpu
        # Initialize PR curve for validation and test
    ...
    def setup(self, trainer: Trainer, pl_module: LightningModule, stage: str) -> None:
        ...
        if trainer.num_devices > 1:
            ...
            self.metrics_to_cpu(self.val_metrics)
            self.metrics_to_cpu(self.test_metrics)
            self.val_pr_curve.to('cpu')
            self.test_pr_curve.to('cpu')
    ...
    def on_validation_batch_end(
        ...
        for metric in self.val_metrics:
            metric.update(batch)
        # Update PR curve with pred_score and gt_label as attributes
        try:
            self.val_pr_curve.update(batch.pred_score, batch.gt_label.int())
        except AttributeError as e:
            logger.error(f"Batch attributes missing: {e}. Available attributes: {dir(batch)}")
            raise
    ...
    def on_validation_epoch_end(
        ...
        """Compute and log validation metrics, and print precision, recall, thresholds."""
        del trainer, pl_module  # Unused argument.
        for metric in self.val_metrics:
            self.log(metric.name, metric)
        # Compute and print PR curve
        if self.val_pr_curve._update_called:  # Check if PR curve was updated
            ...
            print("=====================================\n")
        self.val_pr_curve.reset()  # Reset for next epoch
    ...
    def on_test_batch_end(
        ...
        # Update PR curve with pred_score and gt_label as attributes
        try:
            self.test_pr_curve.update(batch.pred_score, batch.gt_label.int())
        except AttributeError as e:
            logger.error(f"Batch attributes missing: {e}. Available attributes: {dir(batch)}")
            raise
    ...
    def on_test_epoch_end(
        ...
        """Compute and log test metrics, and print precision, recall, thresholds."""
        del trainer, pl_module  # Unused argument.
        for metric in self.test_metrics:
            self.log(metric.name, metric)
        # Compute and print PR curve
        if self.test_pr_curve._update_called:  # Check if PR curve was updated
            ...
            print("Thresholds:", thresholds)
            print("=============================\n")
        self.test_pr_curve.reset()  # Reset for next epoch
